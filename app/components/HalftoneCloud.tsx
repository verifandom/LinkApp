...
export function HalftoneCloud() {
  ...
  useEffect(() => {
    ...
    const fontSize = isMobile ? 4 : 8;
    const charSpacing = isMobile ? 8 : 6;
    let gridRef: boolean[][] = [];
    let time = 0;
    let mouseX = -1000;
    let mouseY = -1000;
    ...
    const updateSize = () => {
      ...
      if (isMobile) {
        // For mobile, use a smaller canvas for better performance
        canvas.width = 1920;
        canvas.height = 1080;
      } else {
      ...
      for (let row = 0; row < rows; row++) {
        ...
        for (let col = 0; col < cols; col++) {
          if (gridRef[row][col]) {
            ...
            const waveX = Math.sin(time + row * 0.08) * (8 * waveMultiplier);

            // Vertical wave with different frequency
            const waveY = Math.sin(time * 0.8 + col * 0.06) * (6 * waveMultiplier);

            // Combined wave effect with secondary waves
            let offsetX = waveX + Math.sin(time * 0.5 + col * 0.05) * (3 * waveMultiplier);
            let offsetY = waveY + Math.cos(time * 0.6 + row * 0.04) * (3 * waveMultiplier);
